...
unzip -q -d $INSTALL_DIR all-in-one/agent-standalone-pkg 2>/dev/null
...
cp all-in-one/server-all-in-one.xml $INSTALL_DIR/tomcat6/conf/server.xml
...
cp all-in-one/settings-all-in-one.xml $INSTALL_DIR/tomcat6/settings.xml
#wget -O $INSTALL_DIR/tomcat6/conf/context.xml http://tank-public.s3-website-us-east-1.amazonaws.com/context.xml 2>/dev/null
cp all-in-one/context.xml $INSTALL_DIR/tomcat6/conf/context.xml
